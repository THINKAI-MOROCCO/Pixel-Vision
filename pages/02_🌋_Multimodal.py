...
# Function to get allowed model names
def get_allowed_model_names(models_info: dict) -> tuple:
    allowed_models = ["bakllava:latest", "llava:latest","rohithbojja/llava-med-v1.6:latest","AnaM3ake-V1:latest"]
    return tuple(
        model
...
def get_tavily_reference(query):
    api_key = "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"
...
def main():
    ...
    missing_models = set(["bakllava:latest", "llava:latest","rohithbojja/llava-med-v1.6:latest"]) - set(available_models)
    ...
    with col_1.popover("⚙️ Model Management", help="Manage models here"):
        if not available_models:
            ...
            model_to_download = st.selectbox(
                "Select a model to download", ["bakllava:latest", "llava:latest","rohithbojja/llava-med-v1.6:latest"]
            )
            if st.button(f"Download {model_to_download}"):
                try:
                    ollama.pull(model_to_download)
                    st.toast(
                        f"""Downloaded model: {model_to_download}""",
                        icon="✅",
                    )
                    st.rerun()
                ...
                with container2.chat_message("assistant", avatar="🌋"):
                    ...
                    if response.status_code == 200:
                        ...
                        if llava_response:
                            ...
                            # Get reference from Tavily API
                            tavily_reference = get_tavily_reference(llava_response)
                            #if tavily_reference:
                            st.markdown("### Reference:")
                            print(tavily_reference)
                            st.write(tavily_reference["results"][0]["title"])
                            st.write(tavily_reference["results"][0]["url"])
                            
                        else:
                            st.error(
                                f"""Failed to get a response from {selected_model}.""",
                                icon="😳",
                            )
